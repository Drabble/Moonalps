...
export default async function handler(req: any, res: any) {
  ...
  try {
    const revalidate = [
     ...
     () => res.unstable_revalidate('/news'),
     () => res.unstable_revalidate('/tickets'),
     () => res.unstable_revalidate('/info')
    ];
    

    console.log(`Fetching bands`);
    const bandsResponse = await fetch(`${URL}/api/bands`);
    ...
    for(const year of years){
      ...
      revalidate.push(res.unstable_revalidate(`/lineup/${year}`));
    }

    console.log(`Fetching galleries`);
    const galleriesResponse = await fetch(`${URL}/api/galleries`);
    ...
    for(const year of years){
      ...
      revalidate.push(() => res.unstable_revalidate(`/galleries/${year}`));
    }
